...
export class AssessComponent implements OnChanges, OnDestroy, OnInit {
  ...
  ngOnInit(): void {
    this.wgSub = this.workGroupService.workGroup$.subscribe(workGroup => {
  ...
  activate(): void {
    ...
    this.user = this.activeWorkGroup.groupMembers.filter(
      (gm: CrseStudentInGroup) => gm.studentId === userId
    )[0];
    this.user.updateStatusOfPeer();
    this.activeWorkGroup.groupMembers.forEach((gm: CrseStudentInGroup) => {
      const hasComment: boolean = this.user.statusOfPeer[gm.studentId]
        .hasComment;
      const assessComplete: boolean = this.user.statusOfPeer[gm.studentId]
        .assessComplete;
    ...
    this.peers = this.activeWorkGroup.groupMembers.filter(
      gm => gm.studentId !== userId
    );
  }

  comment(recipient: CrseStudentInGroup): any {
    this.spProvider
      .loadComment(recipient)
