...
export class StratComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private spProvider: SpProviderService,
  ...
  createStratEntities(): void {
    this.groupMembers.forEach(gm => {
      this.facultyDataContext.getSingleStrat(
        this.courseId,
        this.workGroupId,
        gm.studentId
      );
  ...
  activate() {
    ...
    this.unstratted = this.groupMembers[0].workGroup.facStratResponses
      .filter(fstr => {
        if (fstr.stratPosition === 0) {
          return true;
        }
      })
      .sort((a: FacStratResponse, b: FacStratResponse) => {
        if (
          a.studentAssessee.studentProfile.person.lastName <
          b.studentAssessee.studentProfile.person.lastName
        ) {
          return -1;
        }
        if (
        ...
        if (
          a.studentAssessee.studentProfile.person.firstName <
          b.studentAssessee.studentProfile.person.firstName
        ) {
          return -1;
        }
        if (
    ...
    this.stratted = this.groupMembers[0].workGroup.facStratResponses
      .filter(fstr => {
        if (fstr.stratPosition !== 0) {
          return true;
        }
      ...
      .sort((a: FacStratResponse, b: FacStratResponse) => {
        if (a.stratPosition < b.stratPosition) {
          return -1;
        }
        if (a.stratPosition > b.stratPosition) {
          return 1;
  ...
  isDirty(): boolean {
    if (this.stratted.length > 0) {
      return this.stratted.some(fstrat =>
        fstrat.entityAspect.entityState.isAddedModifiedOrDeleted()
      );
    }
