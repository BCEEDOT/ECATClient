...
export class CommentsComponent implements OnInit, OnDestroy, OnChanges {
  ...
  activate() {
    ...
    this.members.forEach(mem => {
      if (mem.authorOfComments.length > 0) {
        mem["numRemaining"] = mem.authorOfComments.filter(
          com => com.flag.mpFaculty === null
        ).length;
        mem["apprFlags"] = mem.authorOfComments.filter(
          com => com.flag.mpFaculty === MpCommentFlag.appr
        ).length;
        mem["inapprFlags"] = mem.authorOfComments.filter(
          com => com.flag.mpFaculty === MpCommentFlag.inappr
        ).length;
    ...
    this.viewOnly =
      this.selectedAuthor.workGroup.mpSpStatus === MpSpStatus.underReview
        ? false
        : true;
    ...
    if (this.memsWithComments.some(mem => mem["numRemaining"] > 0)) {
      ...
      this.hasChanges = this.selectedAuthor.workGroup.spComments.some(com =>
        com.flag.entityAspect.entityState.isAddedModifiedOrDeleted()
      );
      if (this.hasChanges) {
        this.facWorkGroupService.commentsComplete(false);
      } else {
  ...
  changeAuthor(sel: CrseStudentInGroup) {
    ...
    this.authoredComments.sort((a: StudSpComment, b: StudSpComment) => {
      if (
        a.recipient.studentProfile.person.lastName >
        b.recipient.studentProfile.person.lastName
      ) {
        return 1;
      }
      if (
      ...
      if (
        a.recipient.studentProfile.person.firstName >
        b.recipient.studentProfile.person.firstName
      ) {
        return 1;
      }
      if (
  ...
  checkComplete() {
    this.selectedAuthor.authorOfComments.forEach(com => {
      switch (com.flag.mpFaculty) {
        case MpCommentFlag.appr:
          com["icon"] = "done";
          break;
        case MpCommentFlag.inappr:
          com["icon"] = "highlight_off";
          break;
        default:
          com["icon"] = "";
      }
      ...
      com["displayDate"] =
        com.modifiedDate.toDateString() +
        " " +
        com.modifiedDate.getHours().toString() +
        ":" +
        com.modifiedDate.getMinutes().toString();
    });

    this.selectedAuthor[
      "numRemaining"
    ] = this.selectedAuthor.authorOfComments.filter(
      com => com.flag.mpFaculty === null
    ).length;
    this.selectedAuthor[
      "apprFlags"
  ...
  massSetAll() {
    let allUnflagged = this.members[0].workGroup.spComments.filter(
      com => com.flag.mpFaculty === null
    );
  ...
  massSetAuthor() {
    let authUnflagged = this.authoredComments.filter(
      com => com.flag.mpFaculty === null
    );
    ...
    this.ctx.commit().then(
      ...
      reject => {
        ...
        this.dialogService.openAlert({
          message: "There was a problem saving your flags, please try again.",
          title: "Save Error"
        });
      }
    );
  ...
  discard() {
    let changes = this.selectedAuthor.workGroup.spComments.filter(com =>
      com.flag.entityAspect.entityState.isAddedModifiedOrDeleted()
    );
      ...
      this.dialogService
        .openConfirm({
          message:
            "You have " +
            changes.length +
            " unsaved comment flag changes. Are you sure you want to discard them?",
